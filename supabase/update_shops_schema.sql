-- ============================================
-- UPDATE EXISTING SCHEMA TO MATCH shop-prompt.md
-- ============================================

-- This script updates the existing shops table to match the 4-table structure
-- Run this in Supabase SQL Editor

-- 1. Drop old shops table and recreate with new structure
DROP TABLE IF EXISTS public.shops CASCADE;

-- 2. Create shops table with new structure
...
-- 5. Create shop_printers table
CREATE TABLE IF NOT EXISTS public.shop_printers (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  shop_id uuid NOT NULL,
...
BEGIN
  ...
  IF test_user_id IS NOT NULL THEN
    ...
    INSERT INTO public.shop_printers (
      ...
      (test_shop_id, 'HP LaserJet Pro', 'Laser', 
       ARRAY['B&W Print'], ARRAY['A4', 'Letter'], 'online', now()),
      (test_shop_id, 'Canon PIXMA', 'Inkjet', 
       ARRAY['Color Print'], ARRAY['A4', 'Letter'], 'online', now());

    RAISE NOTICE 'Sample shop created successfully!';
  END IF;
END $$;

-- 10. Update orders table to reference new shops structure
-- (Orders table should already exist from schema.sql, just ensure it works)
