...
export const CommitsGrid = ({ text }: { text: string }) => {
  ...
  const getCellStyle = (index: number, isHighlighted: boolean) => {
    const colorIndex = Math.floor(seededRandom(index) * commitColors.length);
    const delay = (seededRandom(index + 1000) * 0.6).toFixed(1);
    const shouldFlash = !isHighlighted && seededRandom(index + 2000) < 0.3;
    return {
      color: commitColors[colorIndex],
      delay: `${delay}s`,
      shouldFlash,
    };
  };
  ...
  return (
    ...
    >
      {Array.from({ length: gridWidth * gridHeight }).map((_, index) => {
        ...
        const { color, delay, shouldFlash } = getCellStyle(index, isHighlighted);
